...
describe("calculatorService", () => {
  ...
  describe("operator precedence", () => {
    ...
    it("respects PEMDAS — parentheses override", () =>
      expect(calc("(2 + 3) * 4")).toBe(20));
    it("handles nested parentheses", () =>
      expect(calc("((2 + 3) * (1 + 1))")).toBe(10));
    it("chains multiple operators", () =>
      expect(calc("2 + 3 - 1 * 4 / 2")).toBe(3));
  });
  ...
  describe("float precision", () => {
    ...
    it("handles long decimal chains", () =>
      expect(calc("1.1 * 3")).toBeCloseTo(3.3, 10));
  });
