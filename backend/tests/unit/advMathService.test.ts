import {
  ...
  percentage,
  applyAdvFunction,
} from "../../src/services/advMathService";
...
describe("advMathService", () => {
  ...
  describe("applyAdvFunction", () => {
    ...
    it("throws on wrong arity for pow", () => {
      expect(() => applyAdvFunction("pow", [3])).toThrow(
        "pow requires 2 arguments",
      );
    });
    it("throws on wrong arity for sqrt", () => {
      expect(() => applyAdvFunction("sqrt", [4, 2])).toThrow(
        "sqrt requires 1 argument",
      );
    });
    it("throws on unknown function", () => {
      expect(() => applyAdvFunction("tan", [1])).toThrow(
        "Unknown advanced function",
      );
    });
  });
});
