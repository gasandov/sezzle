...
import {
  applyBasicOperator,
  DivisionByZeroError,
} from "../services/basicMathService";
...
export function evaluateRPN(rpn: Token[]): number {
  ...
  for (const token of rpn) {
    if (token.type === "NUMBER") {
      ...
      if (isNaN(n))
        throw new EvaluationError(`Invalid number token: "${token.value}"`);
      stack.push(n);
      continue;
    }
    ...
    if (token.type === "OPERATOR") {
      if (stack.length < 2) {
        throw new EvaluationError(
          `Insufficient operands for operator "${token.value}"`,
        );
      }
      const b = stack.pop()!;
      const a = stack.pop()!;
    ...
    if (token.type === "FUNCTION") {
      ...
      if (stack.length < arity) {
        throw new EvaluationError(
          `Insufficient arguments for function "${fnName}"`,
        );
      }
