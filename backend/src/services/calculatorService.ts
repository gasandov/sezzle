...
export function isFailure(outcome: CalculationOutcome): outcome is CalculationFailure {
...
export function calculate(equation: string): CalculationOutcome {
  ...
  if (!trimmed) {
    return { error: 'Empty equation', detail: 'The equation field must not be empty' };
  }
  ...
  try {
    ...
    if (!isFinite(result)) {
      return { error: 'Result is not finite', detail: 'The equation produced Infinity or NaN' };
    }
